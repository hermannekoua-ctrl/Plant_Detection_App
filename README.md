...
Replace the OpenAI API key before using remote API features (see Security below).
...
How the model was TRAINED //
   ...
   We used 3 CNN layers with the hope of redusing underfitting and overfitting errors. we wanted to go with a large dataset of 19go. After uploading the datasets on two different google drive accounts, 
...
- Replace placeholder `labelMap` with real labels.
- Remove the embedded API key and add instructions for injecting it securely.
- Validate model input/output shapes and adjust preprocessing if needed.
- Add unit tests around `ModelService` to verify preprocessing and softmax behavior.

If you want, I can update `labelMap` for you (if you provide class names), remove the hard-coded API key and add an environment-based loader, or run a quick model-shape check — tell me which one to do next.
